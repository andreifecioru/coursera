...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def seqOp(acc: Int, article: WikipediaArticle): Int = {
      ...
      if (words.contains(lang)) acc + 1 else acc
    }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap { article =>
      val words = article.text.split(" ")
      langs.filter(lang => words.contains(lang)).map(_ -> Seq(article))
    }.reduceByKey(_ ++ _).mapValues(_.toIterable)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap { article =>
      val words = article.text.split(" ")
      langs.filter(lang => words.contains(lang)).map(_ -> 1)
    }.reduceByKey(_ + _).collect().sortBy(- _._2).toList
  }

  def main(args: Array[String]) {

    wikiRdd.take(1).foreach(println)
