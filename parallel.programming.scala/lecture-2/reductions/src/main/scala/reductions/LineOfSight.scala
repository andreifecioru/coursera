...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def tg(i: Int): Float = if (i == 0) 0 else input(i) / i
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
      tree: Tree): Unit = {
    tree match {
      case Leaf(f, t, v) => downsweepSequential(input, output, startingAngle, f, t)
      case Node(l, r) =>
        parallel(
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    def tg(i: Int): Float = input(i) / i

    output(0) = tg(0)
    val upsweepTree = upsweep(input, 0, input.length, threshold)
    downsweep(input, output, output(0), upsweepTree)
  }
}
