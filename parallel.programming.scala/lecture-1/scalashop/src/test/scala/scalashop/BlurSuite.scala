...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should not leak between channels in a uniform image and should not lose precision") {
    val src = new Img(3, 4)
    val pixel = 11141290
    src(0, 0) = pixel; src(1, 0) = pixel; src(2, 0) =  pixel
    src(0, 1) = pixel; src(1, 1) = pixel; src(2, 1) =  pixel
    src(0, 2) = pixel; src(1, 2) = pixel; src(2, 2) =  pixel
    src(0, 3) = pixel; src(1, 3) = pixel; src(2, 3) =  pixel

    assert(boxBlurKernel(src, 1, 2, 1) === pixel,
      s"(boxBlurKernel(1, 2, 1) should be pixel, " +
          s"but it's ${boxBlurKernel(src, 1, 2, 1)})")
  }
