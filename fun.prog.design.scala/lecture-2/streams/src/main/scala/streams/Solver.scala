...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.flatMap { case (block, history) => neighborsWithHistory(block, history) }.filter { n => !(explored contains n._1) }
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    val initial = List((startBlock, List.empty))
    from(initial.toStream, Set(startBlock))
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case s if s.isEmpty => List.empty
    case h #:: rest => h._2
  }
}
